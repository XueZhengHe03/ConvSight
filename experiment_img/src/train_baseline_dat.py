...
def train_model(model_name, save_path):
    ...
    train_dataset = ConvergenceDataset('../dataset_new/train')
    val_dataset = ConvergenceDataset('../dataset_new/val')
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)

    model = build_model(model_name).to(device)
    criterion = nn.CrossEntropyLoss(label_smoothing=0.95)
    optimizer = optim.Adam(model.parameters(), lr=5e-6, weight_decay=5e-4)
...
if __name__ == "__main__":
    ...
    save_path = f'../checkpoints/best_{model_name}_dat.pth'
